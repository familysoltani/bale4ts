...
export class ServerConnection {
  ...
  public constructor(botToken: string, options?: any) {
    let socketDataHandler = (packet: string) => {
      if (ServerPacket.isResponse(packet)) {
        ...
        if (
          response instanceof GetDifferenceResponse &&
          SDKConst.UPDATECONFIG.UPDATECONFIG == 'polling'
        ) {
          ...
          for (let update of updates) {
            ...
            SequenceUtil.setLastSeqNumber(
              Number((<ReceivedMessage>update).seq)
            ).then(() => {})
          }
          SequenceUtil.setLastSeqNumber(response.lastSeq)
            .then(() => {})
            .catch(err => {
              Logger.error('Unable to access seq number file. error: ' + err)
              Logger.grayLog('seqNumber', 'Unable to access seq number' + err)
            })
        } else this.onResponseCB(response)
      } else {
        if (
          ServerPacket.isReceivedMessage(packet) &&
          SDKConst.UPDATECONFIG.UPDATECONFIG == 'realTime'
        ) {
          let receivedMessage = new ReceivedMessage(packet)
          this.handleReceivedMessage(receivedMessage)
        } else {
          let status = MessageStatusFactory.build(packet)
          if (status) this.onMessageStatusCB(status)
        }
      }
    }

    this._socketConnection = new SocketConnection(
      botToken,
      socketDataHandler,
      options
    )
  ...
  private handleReceivedMessage(receivedMessage: ReceivedMessage) {
    if (receivedMessage.quotedMessage) {
      if (this.onQuotedCB) this.onQuotedCB(receivedMessage)
      else this.onMessageCB(receivedMessage)
    } else this.onMessageCB(receivedMessage)
  }

  private startPollingService() {
    let poller = () => {
      SequenceUtil.getLastSeqNumber()
        .then((seq: number) => {
          this.send(new GetDifferenceRequest(seq))
            .then(() => {
...
export class SendQueue {
  ...
  public constructor(
    ...
    if (queueFetchInterval) this.queueFetchInterval = queueFetchInterval
    if (retryInterval) this.requestRetryInterval = retryInterval
    if (maxRetryTime) this.requestMaxRetryTime = maxRetryTime
  }
  ...
  public start(): void {
    ...
    let trySendRequest = function(item: PromisePack) {
      This.connection
        .send(item.identifier.toJson(), item.timeout)
        .then(() => {
   ...
   */
  public stop(): void {
    if (this.nextTryTimer != null) clearTimeout(this.nextTryTimer)
  ...
  public push(
    request: Request,
    timeout: number = this.requestMaxRetryTime
  ): Promise<any> {
    let This = this
