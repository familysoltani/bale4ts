...
export class ApiConnection {
      ...
      for (let i = 0; i < this.responsePromises.length; i++) {
        ...
        if (responsePromise.identifier.id == response.id) {
          ...
          if (response instanceof SendMessageResponse) {
            this.updateReadAndReceivedIdentifiers(
              responsePromise.identifier.randomId,
              response.date
            )
  ...
  public send(request: Request, timeout?: number): BalePromises {
    ...
    if (request instanceof SendMessageRequest) {
      ...
      receivedPromise = new Promise<MessageReceivedStatus>(
        (resolve, reject) => {
          this.receivedPromises.push(
            new PromisePack(sendMessageRequest.randomId, resolve, reject)
          )
        }
      )
