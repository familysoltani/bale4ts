...
export class BaleBot {
   ...
   */
  public hears(
    keywords: string[] | RegExp[] | Sensitive,
    callback: (
      message: Message,
      responder: Responder,
      receivedMessage: ReceivedMessage
    ...
    let sensitive: Sensitive = Array.isArray(keywords)
      ? new TextMessageSensitive(<string[]>keywords)
      : keywords
  ...
  public DownloadFile(
    ...
    return new Promise(function(resolve, reject) {
      This.sendRequest(
        new FileDownloadLinkRequest(fileId, fileAccessHash, fileType)
      )
        .then((response: any) => {
  ...
  private setOnMessage(): void {
    this._apiConnection.setOnMessage(
      (message: Message, sender: User, receivedMessage: ReceivedMessage) => {
        ...
        if (activeConv != null) {
          ...
          if (!anyConvStarted) {
            ...
            for (let sensitivePack of this._hearsPacks) {
              if (
                sensitivePack.callIfMatches(message, sender, receivedMessage)
              ) {
...
export class Responder {
  ...
  public reply(message: string | Message): BalePromises {
    ...
    else msg = new TextMessage(message)
