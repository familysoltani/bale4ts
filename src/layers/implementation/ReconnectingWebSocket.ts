...
export class ReconnectingWebSocket {
  ...
  private connect(reconnectAttempt: any) {
    ...
    var timeout = setTimeout(function() {
      if (This.debug || ReconnectingWebSocket.debugAll) {
        Logger.debug(
          'ReconnectingWebSocket \t connection-timeout \t' + This.url
        )
      }
      This.timedOut = true
      This.ws.close()
    ...
    this.ws.onmessage = function(event: any) {
      if (This.debug || ReconnectingWebSocket.debugAll) {
        Logger.debug(
          'ReconnectingWebSocket \t onmessage \t' +
            This.url +
            ' \t ' +
            event.data
        )
    ...
    this.ws.onerror = function(event: any) {
      if (This.debug || ReconnectingWebSocket.debugAll) {
        Logger.debug(
          'ReconnectingWebSocket \t onerror \t' + This.url + ' \t ' + event
        )
      }
      This.onerror(event)
  ...
  public send(data: any): Promise<any> {
    return new Promise((resolve, reject) => {
      if (this.ws) {
        if (this.debug || ReconnectingWebSocket.debugAll)
          Logger.debug(
            'ReconnectingWebSocket \t send \t' + this.url + ' \t ' + data
          )
