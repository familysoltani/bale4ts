...
export class FileMessage extends Message {
  ...
  getJsonObject(): any {
    return {
      ...
      checkSum: 'checkSum',
      algorithm: 'algorithm',
      fileStorageVersion: !this._fileStorageVersion
        ? 1
        : this._fileStorageVersion
    }
  }

  manipulateFromJsonObject(jsonObject: any): void {
    if (jsonObject.caption.text) this._captionText = jsonObject.caption.text
    else this._captionText = ''
    if (jsonObject.fileId) this._fileId = jsonObject.fileId
    if (jsonObject.accessHash) this._accessHash = jsonObject.accessHash
    if (jsonObject.name) this._name = jsonObject.name
    if (jsonObject.fileSize) this._fileSize = jsonObject.fileSize
    if (jsonObject.mimeType) this._mimeType = jsonObject.mimeType
    if (jsonObject.fileStorageVersion !== undefined)
      this._fileStorageVersion = jsonObject.fileStorageVersion
    else this._fileStorageVersion = 1
  }
}
