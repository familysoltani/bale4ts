...
export class ReceivedMessage extends ServerPacket {
  ...
  constructor(jsonPacket: string) {
    ...
    if (jsonObj.body.$type == 'Message') {
      this._peer = new Peer(
        jsonObj.body.peer.$type,
        jsonObj.body.peer.id,
        jsonObj.body.peer.accessHash
      )
      this._sender = new Peer(
        jsonObj.body.sender.$type,
        jsonObj.body.sender.id,
        jsonObj.body.sender.accessHash
      )
  ...
  get peer(): User {
    //Peer type should be more general in the future.
    return this._peer
  }
